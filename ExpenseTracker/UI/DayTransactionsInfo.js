...
const DayTransactionsInfo = ({ data }) => {
    ...
    function renderItem(itemData) {
        return (
            <Pressable onPress={()=>goToDetails(itemData)}>
            <View style={styles.itemsDayContainer} key={itemData._id}>
                ...
                <View style={{marginRight:5}}>
                    <Text style={{fontSize:18,color: theme.opposite}}>{itemData.title}</Text>
                </View>
                </View>
                <View>
    ...
    return (
        ...
        <View style={styles.outerWrapper}>
            <View style={styles.totalDayInfo}>
                ...
                <Text style={{fontSize:16,color:theme.opposite}}>{days[new Date(data[0].date).getDay()]}</Text>
                </View>
